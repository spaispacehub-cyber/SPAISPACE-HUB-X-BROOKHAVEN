-- SpaiSpace DELTA SAFE MOBILE
-- NO API / NO HTTP / NO LOADSTRING
-- Created by 休伊
...
gui.Name = "SpaiSpace_UI"
gui.Parent = game.CoreGui

local frame = Instance.new("Frame", gui)
frame.Size = UDim2.fromScale(0.8,0.4)
frame.Position = UDim2.fromScale(0.1,0.3)
frame.BackgroundColor3 = Color3.fromRGB(25,25,25)

local title = Instance.new("TextLabel", frame)
title.Size = UDim2.fromScale(1,0.3)
title.Text = "SPAISPACE KEY SYSTEM"
...
box.PlaceholderText = "Enter Key (Free_Key)"
box.TextScaled = true
...
box.FocusLost:Connect(function()
    if box.Text == "Free_Key" then
        status.Text = "KEY OK - LOADING MAIN"
        task.wait(1)
        gui:Destroy()
...
-- =====================================================
-- SPAISPACE PREMIUM (FULL VERSION)
-- Paid Hub UI + Key System
-- Original script body preserved 100%
-- =====================================================
...
-- =========================
local JD_CONFIG = {
    api_key = "JUNKIE_DEV_API_KEY_HERE", -- PUT YOUR API KEY HERE
    service = "SpaiSpacePremium",
    provider = "JunkieDevelopment",
}

-- =========================
-- PREMIUM KEY SYSTEM (GATE)
-- =========================
local function PremiumKeyGate()
    local Library = loadstring(game:HttpGet("https://pastefy.app/TYVuPNOS/raw"))()

    local KeyWindow = Library:MakeWindow({
        Title = "SPAISPACE PREMIUM",
        SubTitle = "License Verification",
        LoadText = "Checking premium access...",
        Flags = "spaispace_premium_key"
    })

    local KeyTab = KeyWindow:MakeTab({ Title = "License", Icon = "rbxassetid://7734053495" })
    KeyTab:AddSection({ "Premium Access Required" })
    KeyTab:AddParagraph({ "Security", "Protected by Junkie Development API" })
    ...
    KeyTab:AddButton({
        Name = "Verify Key",
        Callback = function()
            -- Placeholder validation (replace with Junkie Dev API check)
            if type(inputKey) == "string" and #inputKey >= 16 then
                verified = true
                KeyTab:AddParagraph({ "Status", "License verified. Loading hub..." })
            else
                KeyTab:AddParagraph({ "Status", "Invalid license key." })
            end
        end
    })
